...
export default function PizVegUpdate() {
    ...
    console.log(token)
    useEffect(()=>{
        axios.get("http://127.0.0.1:8000/type/pizzaveg/"+params.id,{headers:{
            "Authorization":token
        }}).then((res)=>{
    ...
    const handleQtn = (e) => {
        e.preventDefault();
        axios.put("http://127.0.0.1:8000/type/pizzaveg/"+params.id, {name:name, quantity: qtn }).then(() => {
            setQtn("")
            navigate("/admin/pizzaveg")
        })
    }
    return (
        <>
